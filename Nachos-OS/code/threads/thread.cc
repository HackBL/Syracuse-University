...
Thread::Thread(char* threadName)
{
    ...
    space = NULL;
    pid = kernel->ThreadId++;
    kernel->ProcessTable->Append(this);
}
...
Thread::~Thread()
{
    ...
    kernel->ProcessTable->Remove(this);
    ASSERT(this != kernel->currentThread);
    if (stack != NULL)
	DeallocBoundedArray((char *) stack, StackSize * sizeof(int));
    delete space; // free the addrSpace
}
...
Thread::Yield ()
{
    ...
    // returns the next process to run as chosen by the scheduling algorithm. 
    // For Round Robin, this function simply chooses the thread at the front of the readyList
    nextThread = kernel->scheduler->FindNextToRun();
    if (nextThread != NULL) {
        // places the currentThread on the readyList. 
        // appends the currentThread to the end of the readyList
...
//
//void 
//Thread::setSpace(AddrSpace* newSpace) {
//  space = newSpace;
//}
...
void 
Thread::setUserRegister(int * newReg) {
    for (int i = 0; i < NumTotalRegs; i++)
        userRegisters[i] = newReg[i];

...
main(int argc, char **argv)
{
    ...
    List<char*>* progList = new List<char*>(); // Task 2!!!!!    Add User Programs into List
    ...
    kernel->Initialize(isRandom, useTLB);
    ...
    // Task 2 
    // cout << "size: " << sizeof(progList) << endl; //Why it's always 4? 
    ...
    //     {
    //         Thread* th = new Thread("User Program Thread ");
    //         th->Fork((VoidFunctionPtr)RunUserProg, (void*)userProgName[i]);
    //     }
    // }
    ...
    // if (!progList->IsEmpty()) {
    //     ListIterator<char*>* items = new ListIterator<char*>(progList);
    //     // isDone: return TRUE if we are at the end of the list
    //     while(!items->IsDone()){
    //         char* progName = items->Item();  // ../test/Write

    //         Thread* progThread = new Thread(progName);

    //         // progThread->Fork((VoidFunctionPtr)RunUserProg, (void*)progName);
    //         progThread->Fork((VoidFunctionPtr)RunUserProg, (void*)progName);

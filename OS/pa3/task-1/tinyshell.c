...
unsigned char process_builtin_commands(cmd_string *C)
{
  if (strcmp(C->cmd, "cd") == 0)
  {
    ...
    // YCGH: Write code for processing the command
    	if (C->arg_count == 1) {		// HOME directory
		if (getenv("HOME") != NULL) {
		...
		if (chdir(C->args[1]) != 0 ) {
			PRINT_ERROR_SYSCALL("chdir");	// No dir exists
		}
	} else {	// Error more than 2 args
		PRINT_ERROR;
	}
  ...
  else if (strcmp(C->cmd, "pwd") == 0)
  {
	...
	if (C->arg_count == 1) {
		if (getcwd(C->cmd, sizeof(C->cmd)) != NULL ) {
			printf("%s\n", C->cmd);
		} else {
      ...
      if (C.redirection == 1)
      {
         ...
         */
     	...
     	close(STDOUT_FILENO);
      	...
      	if (open(C.ofile, O_CREAT|O_WRONLY|O_TRUNC, S_IRWXU) < 0) {		
          PRINT_ERROR_SYSCALL("open");                                            	  FREE_ALLOCATED_MEMORY(C);
          exit(1);
      	}
      }
    ...
    else
    {
      ...
      if (C.detach == 0)
      {
        ...
        // using PRINT_ERROR_SYSCALL("waitpid");
      	...
      	w = waitpid(fc, &status, WUNTRACED | WCONTINUED);
